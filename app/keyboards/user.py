...
def categories_kb(categories: list[Category]) -> InlineKeyboardMarkup:
    ...
    for cat in categories:
        builder.button(text=cat.name, callback_data=f"cat:{cat.id}:0:0")
    builder.adjust(2)
...
def single_product_kb(product_id: int) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text="Подробнее ▾", callback_data=f"prod:{product_id}")
    return builder.as_markup()
...
def feed_controls_kb(
    ...
    if brand_id:
        filter_row.button(
            text=f"✖️ Бренд: {brand_name} — сбросить",
            callback_data=f"cat:{category_id}:0:0",
        )
        filter_row.adjust(1)
        builder.attach(filter_row)
    elif show_brand_filter:
        filter_row.button(text="🔍 Фильтр по бренду", callback_data=f"cf:{category_id}")
        filter_row.adjust(1)
        builder.attach(filter_row)

    back_row = InlineKeyboardBuilder()
    back_row.button(text="⬅️ К категориям", callback_data="catalog")
...
def brand_filter_kb(brands: list[Brand], category_id: int) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    for br in brands:
        builder.button(text=br.name, callback_data=f"cat:{category_id}:{br.id}:0")
    builder.adjust(2)
    actions = InlineKeyboardBuilder()
    actions.button(text="📋 Все бренды", callback_data=f"cat:{category_id}:0:0")
    actions.button(text="⬅️ К категориям", callback_data="catalog")
    actions.adjust(1)
...
def product_card_kb(product: Product, admin_username: str) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text="🛒 Купить", url=buy_deeplink(product, admin_username))
    builder.button(text="⬅️ К списку", callback_data=f"cat:{product.category_id}:0:0")
    builder.button(text="🏠 В меню", callback_data="close")
    builder.adjust(1)
    return builder.as_markup()
